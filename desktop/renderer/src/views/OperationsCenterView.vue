...
const customBackup = reactive({ directory: '', available: false, loading: false })
...
async function exportBackup(name: string) { try { const result = await window.agricultureDesktop?.exportBackup(auth.token, name); if (result && !result.canceled) message.success('备份已导出到所选位置') } catch (cause) { message.error(cause instanceof Error ? cause.message : '导出备份失败') } }
...
async function backupToCustomDirectory() {
  ...
  try {
    ...
    if (result) message.success(`镜像备份已创建：${result.name}`)
  } catch (cause) {
    ...
    <n-tabs type="line" animated>
      ...
      <n-tab-pane name="data" tab="数据安全">
        <div class="data-safety-grid">
          <section class="data-tool"><span class="data-tool-icon"><DatabaseBackup /></span><div><h3>新建本地数据库备份</h3><p>使用 SQLite `VACUUM INTO` 生成一致性快照，保存在本机数据目录。</p></div><n-button type="primary" :loading="operations.loading.backupMutation" @click="createBackup">立即备份</n-button></section>
          <section class="data-tool"><span class="data-tool-icon amber"><ShieldAlert /></span><div><h3>数据库完整性</h3><p>执行 `PRAGMA integrity_check`，检查页结构和索引一致性。</p><small v-if="operations.integrity">上次检查 {{ new Date(operations.integrity.checkedAt).toLocaleString('zh-CN', { hour12: false }) }} · {{ operations.integrity.ok ? '通过' : '异常' }}</small></div><n-button secondary :loading="operations.loading.integrity" @click="checkIntegrity">运行检查</n-button></section>
           <section class="data-tool local-sync-tool"><span class="data-tool-icon blue"><FileUp /></span><div><h3>选择数据文件本地同步</h3><p>选择其他本机备份或 SQLite 数据文件，校验后按更新时间合并业务数据，并保留现有云端、局域网和终端同步配置。</p><small v-if="operations.lastLocalSync">最近同步 {{ operations.lastLocalSync.sourceName }} · 新增 {{ operations.lastLocalSync.inserted }} · 更新 {{ operations.lastLocalSync.updated }}</small></div><n-button secondary type="info" :disabled="!isDesktop" :loading="operations.loading.localSync" @click="selectLocalDataFile">选择数据文件</n-button></section>
           <section class="data-tool local-sync-tool"><span class="data-tool-icon"><FolderOpen /></span><div><h3>指定目录镜像备份</h3><p>选择项目目录、移动硬盘或共享盘作为额外镜像位置；程序受控备份和其他同步方式会继续保留。</p><small>{{ customBackup.directory || '尚未选择目录，例如 E:\\Project\\agriculture' }}</small></div><div class="custom-backup-actions"><n-button secondary :disabled="!isDesktop" @click="selectCustomBackupDirectory">选择目录</n-button><n-button type="primary" :disabled="!customBackup.available" :loading="customBackup.loading" @click="backupToCustomDirectory">镜像备份</n-button></div></section>
         </div>
