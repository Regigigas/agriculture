...
const farm = useFarmStore(); const production = useProductionStore(); const message = useMessage(); const showModal = ref(false); const formRef = ref<FormInst | null>(null)
...
<template>
  ...
  <state-panel :loading="farm.loading.fields" :error="farm.errors.fields" :empty="!farm.fields.length" empty-text="尚未建立农田档案" @retry="farm.loadFields">
    <div class="field-grid"><n-card v-for="field in farm.fields" :key="field.id"><div class="field-card-head"><span class="field-icon"><Sprout /></span><n-tag size="small" :bordered="false" :type="field.status === 'healthy' ? 'success' : field.status === 'attention' ? 'warning' : 'default'">{{ field.status === 'healthy' ? '长势良好' : field.status === 'attention' ? '需关注' : '休耕' }}</n-tag></div><h3>{{ field.name }}</h3><p class="crop-name">{{ farmName(field.farmId) }} · {{ field.crop || '暂未种植' }}</p><div class="field-meta"><span><Ruler />{{ field.area }} 亩 · 墒情 {{ field.soilMoisture }}%</span><span><MapPin />{{ field.location }}</span><span><UserRound />{{ field.manager }}</span></div><div class="planted-date">种植 {{ field.plantedAt }} · 预计采收 {{ field.expectedHarvestAt }}</div></n-card></div>
  </state-panel>
