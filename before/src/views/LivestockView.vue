...
import type { HealthRecordType, LivestockBatch, LivestockSpecies } from '@/types'
...
const filteredBatches = computed(() => livestock.batches.filter((item) => {
  const keyword = batchKeyword.value.trim().toLowerCase()
  return (!keyword || `${item.code} ${item.breed} ${item.manager} ${barnName(item.barnId)}`.toLowerCase().includes(keyword))
    && (!batchStatus.value || item.status === batchStatus.value)
    && (!batchBarnId.value || item.barnId === batchBarnId.value)
}))
const quarantineBatches = computed(() => livestock.batches.filter((item) => item.status === 'quarantine'))
const dueHealthRecords = computed(() => livestock.health.filter((item) => item.nextDueDate && item.nextDueDate <= today()).slice(0, 3))
...
function open(type: typeof modal.value) {
  ...
  if (type === 'barn' && !barnForm.farmId) barnForm.farmId = farmOptions.value[0]?.value || ''
  if (type === 'batch' && !batchForm.barnId) {
    batchForm.barnId = barnOptions.value.find((item) => item.value === selectedBarnId.value)?.value || barnOptions.value[0]?.value || ''
    syncBatchSpecies(batchForm.barnId)
  }
  const preferredBatch = activeBatches.value.find((item) => item.barnId === selectedBarnId.value) || activeBatches.value[0]
  if (preferredBatch) {
    if (type === 'feeding' && !feedingForm.batchId) feedingForm.batchId = preferredBatch.id
    if (type === 'health' && !healthForm.batchId) healthForm.batchId = preferredBatch.id
    if (type === 'exit' && !exitForm.batchId) { exitForm.batchId = preferredBatch.id; exitForm.quantity = preferredBatch.quantity; exitForm.averageWeight = preferredBatch.averageWeight }
  }
  modal.value = type
}
...
function openForBatch(type: 'feeding' | 'health' | 'exit', batch: LivestockBatch) {
  selectedBarnId.value = batch.barnId
  if (type === 'feeding') feedingForm.batchId = batch.id
  if (type === 'health') healthForm.batchId = batch.id
  if (type === 'exit') { exitForm.batchId = batch.id; exitForm.quantity = batch.quantity; exitForm.averageWeight = batch.averageWeight }
  open(type)
}

function syncExitBatch(batchId: string) {
  const batch = livestock.batches.find((item) => item.id === batchId)
  if (batch) { exitForm.quantity = batch.quantity; exitForm.averageWeight = batch.averageWeight }
}

function environmentRisk(barnId: string) {
  const barn = livestock.barns.find((item) => item.id === barnId)
  if (!barn) return ''
  if (remainingCapacity(barnId) < 0) return '超出设计容量'
  if (barn.temperature < 8 || barn.temperature > 30) return '温度异常'
  if (barn.humidity < 35 || barn.humidity > 80) return '湿度异常'
  return ''
}
...
onMounted(async () => {
  await Promise.allSettled([livestock.loadAll(), production.loadFarms()])
  selectedBarnId.value = livestock.barns[0]?.id || ''
})
</script>
...
<template>
  ...
  <section class="livestock-workbench">
    <div class="workbench-title"><div><strong>今日工作台</strong><span>常用操作和异常事项集中处理</span></div><n-button text type="primary" @click="livestock.loadAll">刷新数据</n-button></div>
    <div class="quick-actions">
      <button type="button" @click="goTo('feeding', 'feeding')"><Boxes /><span><strong>登记饲喂</strong><small>自动选择当前圈舍批次</small></span></button>
      <button type="button" @click="goTo('health', 'health')"><ClipboardCheck /><span><strong>健康防疫</strong><small>{{ livestock.summary.healthDue }} 项近期待办</small></span></button>
      <button type="button" @click="goTo('batches', 'batch')"><Sprout /><span><strong>批次入场</strong><small>建立新存栏批次</small></span></button>
      <button type="button" @click="goTo('exits', 'exit')"><Truck /><span><strong>登记出栏</strong><small>同步扣减并生成追溯码</small></span></button>
    </div>
    <div v-if="quarantineBatches.length || dueHealthRecords.length" class="attention-strip">
      <AlertTriangle />
      <strong>需要关注</strong>
      <button v-if="quarantineBatches.length" type="button" @click="activeTab = 'batches'; batchStatus = 'quarantine'">{{ quarantineBatches.length }} 个隔离批次</button>
      <button v-if="dueHealthRecords.length" type="button" @click="activeTab = 'health'">{{ dueHealthRecords.length }} 项防疫已到期</button>
    </div>
  </section>

  <livestock-scene-3-d v-if="showScene" :barns="livestock.barns" :batches="livestock.batches" :selected-barn-id="selectedBarnId" @select="selectBarn" />

  <state-panel :loading="livestock.loading.all" :error="livestock.errors.all" @retry="livestock.loadAll">
    <n-tabs v-model:value="activeTab" type="line" animated>
      <n-tab-pane name="barns" :tab="`圈舍环境 ${livestock.barns.length}`">
        <div class="section-actions"><n-button type="primary" size="small" @click="open('barn')"><template #icon><Plus /></template>新增圈舍</n-button></div>
        <div class="barn-grid">
          <article v-for="item in livestock.barns" :key="item.id" :class="['barn-card', item.status, { selected: selectedBarnId === item.id }]" @click="selectBarn(item.id)">
            ...
            <p v-if="environmentRisk(item.id)" class="barn-warning"><AlertTriangle />{{ environmentRisk(item.id) }}</p>
            ...
            <footer><span>负责人 {{ item.manager }}</span><n-select size="tiny" :value="item.status" :options="[{label:'运行中',value:'active'},{label:'维护中',value:'maintenance'},{label:'空置',value:'empty'}]" @click.stop @update:value="livestock.updateBarnStatus(item.id, $event)" /></footer>
          </article>
        </div>
      </n-tab-pane>
      <n-tab-pane name="batches" :tab="`存栏批次 ${activeBatches.length}`">
        <div class="filter-bar">
          <n-input v-model:value="batchKeyword" clearable placeholder="搜索批次、品种、负责人" />
          <n-select v-model:value="batchBarnId" clearable placeholder="全部圈舍" :options="livestock.barns.map(item => ({ label: item.name, value: item.id }))" />
          <n-select v-model:value="batchStatus" clearable placeholder="全部状态" :options="[{label:'养殖中',value:'active'},{label:'隔离检疫',value:'quarantine'},{label:'已结束',value:'exited'}]" />
          <n-button secondary @click="resetBatchFilters">重置</n-button>
          <n-button type="primary" @click="open('batch')"><template #icon><Plus /></template>批次入场</n-button>
        </div>
        <div class="table-wrap"><n-table :single-line="false"><thead><tr><th>批次</th><th>圈舍</th><th>品种</th><th>存栏</th><th>均重</th><th>计划出栏</th><th>状态</th><th>快捷操作</th></tr></thead><tbody><tr v-for="item in filteredBatches" :key="item.id"><td><strong>{{ item.code }}</strong><small class="cell-detail">负责人 {{ item.manager }}</small></td><td>{{ barnName(item.barnId) }}</td><td>{{ speciesLabels[item.species] }} · {{ item.breed }}</td><td>{{ item.quantity }} 头/只</td><td>{{ item.averageWeight }} kg</td><td>{{ item.targetExitDate }}</td><td><n-select v-if="item.status !== 'exited'" size="tiny" :value="item.status" :options="[{label:'养殖中',value:'active'},{label:'隔离检疫',value:'quarantine'}]" @update:value="livestock.updateBatchStatus(item.id, $event)" /><n-tag v-else size="small">已结束</n-tag></td><td><div class="row-actions"><n-button size="tiny" text type="primary" :disabled="item.status === 'exited'" @click="openForBatch('feeding', item)">饲喂</n-button><n-button size="tiny" text type="primary" :disabled="item.status === 'exited'" @click="openForBatch('health', item)">防疫</n-button><n-button size="tiny" text type="primary" :disabled="item.status === 'exited'" @click="openForBatch('exit', item)">出栏</n-button></div></td></tr><tr v-if="!filteredBatches.length"><td colspan="8" class="empty-cell">没有符合条件的养殖批次</td></tr></tbody></n-table></div>
      </n-tab-pane>
      <n-tab-pane name="feeding" :tab="`饲喂记录 ${livestock.feeding.length}`"><div class="section-actions"><n-button size="small" type="primary" @click="open('feeding')">登记饲喂</n-button></div><div class="table-wrap"><n-table><thead><tr><th>时间</th><th>批次</th><th>饲料</th><th>数量</th><th>操作人</th><th>备注</th></tr></thead><tbody><tr v-for="item in livestock.feeding" :key="item.id"><td>{{ new Date(item.fedAt).toLocaleString('zh-CN') }}</td><td>{{ batchName(item.batchId) }}</td><td>{{ item.feedName }}</td><td>{{ item.quantityKg }} kg</td><td>{{ item.operator }}</td><td>{{ item.notes || '—' }}</td></tr></tbody></n-table></div></n-tab-pane>
      <n-tab-pane name="health" :tab="`健康防疫 ${livestock.health.length}`"><div class="section-actions"><n-button size="small" type="primary" @click="open('health')">登记健康事件</n-button></div><div class="table-wrap"><n-table><thead><tr><th>日期</th><th>批次</th><th>类型</th><th>事项</th><th>涉及数量</th><th>兽医/负责人</th><th>下次日期</th></tr></thead><tbody><tr v-for="item in livestock.health" :key="item.id"><td>{{ item.occurredAt }}</td><td>{{ batchName(item.batchId) }}</td><td>{{ healthLabels[item.type] }}</td><td>{{ item.title }}</td><td>{{ item.affectedQuantity }}</td><td>{{ item.veterinarian }}</td><td>{{ item.nextDueDate || '—' }}</td></tr></tbody></n-table></div></n-tab-pane>
      <n-tab-pane name="exits" :tab="`出栏追溯 ${livestock.exits.length}`"><div class="section-actions"><n-button size="small" type="primary" @click="open('exit')"><template #icon><Truck /></template>登记出栏</n-button></div><div class="table-wrap"><n-table><thead><tr><th>出栏日期</th><th>批次</th><th>数量</th><th>均重</th><th>去向</th><th>检疫员</th><th>追溯码</th></tr></thead><tbody><tr v-for="item in livestock.exits" :key="item.id"><td>{{ item.exitedAt }}</td><td>{{ batchName(item.batchId) }}</td><td>{{ item.quantity }}</td><td>{{ item.averageWeight }} kg</td><td>{{ item.destination }}</td><td>{{ item.inspector }}</td><td><code>{{ item.traceCode }}</code></td></tr><tr v-if="!livestock.exits.length"><td colspan="7" class="empty-cell">暂无出栏记录</td></tr></tbody></n-table></div></n-tab-pane>
    </n-tabs>
  </state-panel>
  ...
  <n-modal :show="modal === 'exit'" preset="card" title="登记出栏与追溯" class="form-modal" @update:show="!$event && (modal = '')"><n-form label-placement="top"><div class="form-grid"><n-form-item label="养殖批次"><n-select v-model:value="exitForm.batchId" :options="batchOptions" @update:value="syncExitBatch" /></n-form-item><n-form-item label="出栏数量"><n-input-number v-model:value="exitForm.quantity" :min="1" :max="livestock.batches.find(item => item.id === exitForm.batchId)?.quantity" /></n-form-item><n-form-item label="平均体重 kg"><n-input-number v-model:value="exitForm.averageWeight" :min="0.01" /></n-form-item><n-form-item label="出栏日期"><n-date-picker v-model:formatted-value="exitForm.exitedAt" value-format="yyyy-MM-dd" type="date" /></n-form-item><n-form-item label="去向"><n-input v-model:value="exitForm.destination" /></n-form-item><n-form-item label="检疫员"><n-input v-model:value="exitForm.inspector" /></n-form-item></div><n-form-item label="备注"><n-input v-model:value="exitForm.notes" /></n-form-item><div class="modal-actions"><n-button @click="modal=''">取消</n-button><n-button type="primary" @click="submit('exit')">确认出栏</n-button></div></n-form></n-modal>
</template>

<style scoped>
.livestock-kpis { grid-template-columns: repeat(5, minmax(0, 1fr)); }
.livestock-workbench { margin-bottom: 18px; padding: 16px; border: 1px solid #dfe6e1; border-radius: 9px; background: #fff; }
.workbench-title { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; }.workbench-title > div { display: flex; align-items: baseline; gap: 10px; }.workbench-title strong { color: #35483e; font-size: 14px; }.workbench-title span { color: #849088; font-size: 10px; }
.quick-actions { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 10px; }.quick-actions button { display: flex; align-items: center; gap: 10px; min-width: 0; padding: 12px; border: 1px solid #e2e8e4; border-radius: 7px; color: #3f5549; background: #f8faf8; text-align: left; cursor: pointer; transition: .16s ease; }.quick-actions button:hover { border-color: #8fb09a; background: #f0f6f2; transform: translateY(-1px); }.quick-actions svg { flex: 0 0 auto; width: 22px; color: #568068; }.quick-actions span { min-width: 0; }.quick-actions strong, .quick-actions small { display: block; }.quick-actions strong { font-size: 12px; }.quick-actions small { margin-top: 3px; overflow: hidden; color: #89958e; font-size: 9px; text-overflow: ellipsis; white-space: nowrap; }
.attention-strip { display: flex; align-items: center; gap: 9px; margin-top: 12px; padding: 9px 11px; border-radius: 6px; color: #795b29; background: #fff7e7; font-size: 10px; }.attention-strip svg { width: 16px; }.attention-strip button { padding: 3px 7px; border: 1px solid #e8c98d; border-radius: 10px; color: #795b29; background: #fff; cursor: pointer; }
.section-actions { margin-bottom: 12px; display: flex; justify-content: flex-end; }
.filter-bar { display: grid; grid-template-columns: minmax(190px, 1fr) 180px 150px auto auto; gap: 9px; margin-bottom: 12px; }.filter-bar .n-button { white-space: nowrap; }
.barn-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 14px; }.barn-card { padding: 17px; border: 1px solid #dfe6e1; border-radius: 8px; background: #fff; cursor: pointer; transition: .16s ease; }.barn-card:hover { border-color: #9bb6a4; box-shadow: 0 5px 16px rgba(52,76,63,.08); }.barn-card.selected { border-color: #5d876b; box-shadow: 0 0 0 2px rgba(93,135,107,.12); }.barn-card.maintenance { border-color: #e5d3ab; }.barn-card.empty { opacity: .65; }.barn-card > div:first-child { display: flex; align-items: center; justify-content: space-between; }.barn-card > div:first-child > span { width: 38px; height: 38px; display: grid; place-items: center; border-radius: 7px; background: #e8f1eb; color: #48705a; }.barn-card svg { width: 19px; }.barn-card h3 { margin: 12px 0 4px; color: #33443b; font-size: 15px; }.barn-card > p { margin: 0; color: #7b8781; font-size: 10px; }.barn-card dl { margin: 15px 0; display: grid; grid-template-columns: repeat(4, 1fr); gap: 6px; }.barn-card dl div { padding: 8px 5px; border-radius: 5px; background: #f5f7f5; text-align: center; }.barn-card dt { color: #8a958f; font-size: 9px; }.barn-card dd { margin: 3px 0 0; color: #3b4c43; font-size: 12px; font-weight: 600; }.barn-card footer { padding-top: 11px; border-top: 1px solid #edf0ee; display: grid; grid-template-columns: 1fr 100px; align-items: center; gap: 8px; color: #78847e; font-size: 10px; }
.barn-card > .barn-warning { display: flex; align-items: center; gap: 4px; margin-top: 8px; padding: 5px 7px; border-radius: 4px; color: #9b6828; background: #fff5df; }.barn-card > .barn-warning svg { width: 12px; }.row-actions { display: flex; gap: 7px; white-space: nowrap; }
code { color: #36684a; font-size: 10px; }.empty-cell { padding: 28px !important; color: #8a958f; text-align: center; }
@media (max-width: 1180px) { .livestock-kpis { grid-template-columns: repeat(3, 1fr); }.quick-actions { grid-template-columns: repeat(2, 1fr); }.barn-grid { grid-template-columns: repeat(2, 1fr); }.filter-bar { grid-template-columns: 1fr 1fr; } }
@media (max-width: 720px) { .livestock-kpis, .quick-actions, .barn-grid, .filter-bar { grid-template-columns: 1fr; }.workbench-title span { display: none; }.attention-strip { align-items: flex-start; flex-wrap: wrap; } }
</style>
