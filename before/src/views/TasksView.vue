...
function complete(task: FarmTask) {
  dialog.warning({ title: '确认完成任务', content: `确认将“${task.title}”标记为已完成？`, positiveText: '确认完成', negativeText: '取消', onPositiveClick: async () => {
    try { await farm.updateTaskStatus(task.id, 'completed'); message.success('任务已完成') } catch { message.error(farm.errors.taskMutation) }
  } })
}
