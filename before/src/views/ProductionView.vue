...
const actualCost = computed(() => production.logs.reduce((sum, item) => sum + item.cost, 0) + production.costAdjustments.reduce((sum, item) => sum + (item.type === 'supplement' ? item.amount : -item.amount), 0))
...
function createCycle() {
  ...
  const input = { ...cycleForm, budget: yuanToCents(cycleForm.budget) }
  dialog.warning({
    title: '确认建立种植季',
    content: `确认建立“${input.crop}”种植季，初始成本为 ${money(input.budget)}？初始成本建账后不可修改，如有差错只能通过补录或冲正凭证调整。`,
    positiveText: '确认建账', negativeText: '返回检查',
    onPositiveClick: async () => {
      try { await production.createCycle(input); showCycle.value = false; message.success('种植季已建立，初始成本已锁定') }
      catch { message.error(production.errors.cycleMutation); return false }
    },
  })
}
async function createPlan() {
  if (!planForm.cycleId || !planForm.title.trim() || !planForm.assignee.trim()) return message.warning('请选择种植季并填写计划和负责人')
  try { await production.createPlan({ ...planForm, plannedCost: yuanToCents(planForm.plannedCost) }); showPlan.value = false; message.success('生产计划已建立') } catch { message.error(production.errors.planMutation) }
}
async function createLog() {
  ...
  try {
    await production.createLog({ ...logForm, cost: yuanToCents(logForm.cost), materialQuantity: logForm.inventoryItemId ? logForm.materialQuantity : 0 })
...
<template>
  ...
  <div class="domain-kpis"><div><span class="domain-kpi-icon"><Wheat /></span><small>活跃种植季</small><strong>{{ activeCycles }}</strong><em>个</em></div><div><span class="domain-kpi-icon blue"><CalendarRange /></span><small>初始成本</small><strong>{{ money(plannedBudget) }}</strong><em>建季时登记且不可修改</em></div><div><span class="domain-kpi-icon amber"><History /></span><small>过程成本</small><strong>{{ money(actualCost) }}</strong><em>{{ production.logs.length }} 条实绩 + {{ production.costAdjustments.length }} 条凭证</em></div></div>
