...
const OPERATION_CONFIRMATIONS: Readonly<Record<string, string>> = {
...
export class AuthService {
  ...
  register(input: Record<string, unknown>): { token: string; user: User } {
    if (process.env.ALLOW_PUBLIC_REGISTRATION !== 'true') throw new ForbiddenException('公开注册未启用');
    const user = this.createUser(input, 'worker');
    return { token: this.createSession(user.id), user };
  }

  createUser(input: Record<string, unknown>, forcedRole?: 'worker'): User {
    const username = this.normalizeUsername(input.username);
    const password = this.normalizePassword(input.password, true);
    const name = this.normalizeName(input.name);
    const roleValue = forcedRole ?? input.role ?? 'worker';
    if (roleValue !== 'admin' && roleValue !== 'worker') throw new BadRequestException('用户角色无效');
    const id = randomUUID();
    ...
    try {
      this.database.connection.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, username, this.hashPassword(password, salt), salt, name, roleValue, now, now);
    } catch (error) {
      if (error instanceof Error && error.message.includes('UNIQUE')) throw new ConflictException('该用户名已被使用');
      throw error;
    }
    return { id, username, name, role: roleValue };
  }
  ...
  createOperationAuthorization(
    ...
    if (user.role !== 'admin') throw new ForbiddenException('仅管理员可授权高危操作');
    const operation = typeof operationValue === 'string' ? operationValue : '';
    const expectedConfirmation = OPERATION_CONFIRMATIONS[operation];
    if (!expectedConfirmation) throw new BadRequestException('不支持的高危操作');
    if (confirmationValue !== expectedConfirmation) throw new BadRequestException('确认短语不正确');
    const password = this.normalizePassword(passwordValue, false);
    const row = this.userById(user.id);
    if (!row || !this.passwordMatches(password, row.passwordSalt, row.passwordHash)) {
      throw new UnauthorizedException('当前密码错误');
    }
    ...
    this.database.transaction(() => {
      this.database.connection.prepare('DELETE FROM operation_authorizations WHERE expires_at <= ?')
        .run(createdAt.toISOString());
      this.database.connection.prepare(`
        INSERT INTO operation_authorizations (token_hash, user_id, operation, expires_at, created_at)
        VALUES (?, ?, ?, ?, ?)
      `).run(this.tokenHash(token), user.id, operation, expiresAt, createdAt.toISOString());
    });
    return { token, operation, expiresAt };
  }

  consumeOperationAuthorization(userId: string, operation: string, token: string): void {
    if (!token) throw new ForbiddenException('缺少高危操作授权');
    const consumed = this.database.transaction(() => this.database.connection.prepare(`
      DELETE FROM operation_authorizations
      WHERE token_hash = ? AND user_id = ? AND operation = ? AND expires_at > ?
    `).run(this.tokenHash(token), userId, operation, new Date().toISOString()).changes);
    if (consumed !== 1) throw new ForbiddenException('高危操作授权无效、已使用或已过期');
  }

  revoke(token: string): void {
    if (token === process.env.DEMO_TOKEN) return;
  ...
  private createSession(userId: string): string {
    const token = randomBytes(32).toString('base64url');
    const createdAt = new Date();
    const expiresAt = new Date(createdAt.getTime() + 30 * 24 * 60 * 60_000);
